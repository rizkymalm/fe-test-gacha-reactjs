...
const CardBasic = ({
    ...
    return (
        <div
            className={`relative rounded-xl border-2 border-accent-light/40 bg-bg-light-1 ${padding === 'loose' ? 'p-3' : 'p-0'} text-text-light-primary shadow-custom-light drop-shadow-lg dark:border-accent-dark/20 dark:bg-bg-dark-1 dark:text-text-dark-primary ${colSpan && colSpan}`}
            {...props}
        >
            {title || subtitle ? (
