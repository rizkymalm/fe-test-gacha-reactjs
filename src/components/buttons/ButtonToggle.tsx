...
const ButtonToggle = ({ isChecked, ...props }: Props) => {
    ...
    return (
        ...
        >
            ...
            />
            <div
                className={`relative flex h-[46px] w-[82px] items-center justify-center rounded-md shadow-card ${!isChecked ? 'bg-white' : 'bg-black'}`}
            >
                <div
                    className={`absolute inset-y-0 left-0 z-1 m-auto size-9 rounded-md transition-transform duration-300 ${!isChecked ? 'translate-x-1 bg-blue-400' : 'translate-x-10 bg-blue-900'}`}
                />
                <span
                    className={`relative z-9 flex size-9 items-center justify-center rounded ${
